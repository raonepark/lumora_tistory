...
document.addEventListener('DOMContentLoaded', () => {
    console.log('Script v30 Loaded'); // Updated Version
...
function updateCommentAvatars() {
    ...
    comments.forEach(comment => {
        ...
        // 1. Identify Admin or Member explicitly
        const isAdmin = comment.classList.contains('rp_admin');
        const isMember = comment.classList.contains('rp_member'); // Logged in users often have this

        // 2. Identify "Real" Profile Images (Tistory/Kakao hosted user uploads)
        // Default profiles often have 'img_profile_default' or 'account_default' in URL
        // Real profiles usually do NOT have 'default' in the filename, or are from distinct CDN paths.
        // However, safest bet is: If Admin/Member, TRUST their avatar.

        if (isAdmin || isMember) {
            // Restore original styling (Round, no padding background)
        ...
        // 3. For Guests (or if we failed to detect member), use the Generic Icon
        // This makes all anonymous users look identical.
