$(document).ready(function () {
  ...
  */
  ...
  (function () {
    ...
    var CLICK_SELECTOR =
      '#detail_page .container_postbtn [id^="reaction-"] .btn_post, ' +
      '#detail_page .container_postbtn [id^="reaction-"] a, ' +
      '#detail_page .container_postbtn [id^="reaction-"] button';
    ...
    function isLiked($wrap, $clicked) {
      if (isActive($clicked)) return true;
      if (!$wrap || !$wrap.length) return false;
      if (isActive($wrap)) return true;
      return $wrap.find('[aria-pressed="true"], .on, .active').length > 0;
    }
    ...
    $(document)
      ...
      .on("click.lumoraLikeAnim", CLICK_SELECTOR, function () {
        ...
        if (isLiked($wrap, $clicked)) return;
        animate($wrap);
      });
  })();
  ...
  (function () {
    ...
    function extractDescriptionFromHtml(html) {
      ...
      try {
        ...
        if (!desc) {
          var firstParagraph =
            doc.querySelector("#detail_page .article .contents_style p") ||
            doc.querySelector("article p") ||
            doc.querySelector("p");
          desc = normalizeText(firstParagraph ? firstParagraph.textContent : "");
        }
