...
// ==============================
// Category Accordion & Polish (Moved from HTML)
// ==============================
(function () {
    function initAccordion() {
        var containers = ['#sidebar', '.m-cat-body'];
        containers.forEach(function (containerSelector) {
            ...
            listItems.forEach(function (li) {
                ...
                if (!link.querySelector('.cat-count')) {
                    function getName(el) {
                        return el.textContent.replace(/\s+/g, ' ').replace(/\(\d+\)\s*$/, '').trim();
                    }
                ...
                if (link.textContent.indexOf('분류 전체보기') !== -1) return;

                var subContainer = li.querySelector('ul');
                if (subContainer) {
                    subContainer.classList.add('sub-category-item');
                    link.classList.add('accordion-parent');

                    if (!subContainer.querySelector('.view-all-item')) {
                        var pCount = getCount(link);
                        ...
                        viewAllLink.innerHTML = '전체 글 <span class="cat-count">(' + pCount + ')</span>';
                        viewAllLi.appendChild(viewAllLink);

                        if (subContainer.firstChild) {
                            subContainer.insertBefore(viewAllLi, subContainer.firstChild);
                        } else {
                            subContainer.appendChild(viewAllLi);
                        }
                    }

                    if (!link.dataset.accordionInit) {
    ...
    function polishCategoryText() {
        var catNodes = document.querySelectorAll('.list_content .category, .titleWrap .category a');
        catNodes.forEach(function (node) {
            if (node.innerHTML.indexOf('/') !== -1) {
                node.innerHTML = node.innerHTML.replace(/\//g, '&nbsp;<span style="opacity:0.5; font-size:0.9em;">·</span>&nbsp;');
            }
        });
    }
    ...
    function checkEmptyCategory() {
        ...
        var hasList = document.querySelector('.list_content');
        var hasDetail = document.getElementById('detail_page');
        ...
        if (!hasList && !hasDetail) {
            ...
            var catName = '이 카테고리';
            try {
                var parts = window.location.pathname.split('/category/');
                if (parts[1]) {
            ...
            contentArea.innerHTML =
                ...
                '<p class="empty-text">‘<span class="highlight" style="max-width: 60vw; display: inline-block; vertical-align: bottom; text-overflow: ellipsis; overflow: hidden; white-space: nowrap;">' + catName + '</span>’<br/>아직 글이 없어요.</p>' +
                '<a href="/" class="empty-btn">전체 글 보기</a>' +
                '</div>';
        }
    }

    // Comment Pagination V3 (Enhanced with Prev/Next buttons)
    function initCommentPaginationV3() {
        var itemsPerPage = 15;
        var commentLists = document.querySelectorAll('.commentlist');
        if (!commentLists.length) return;

        commentLists.forEach(function (listWrapper, listIndex) {
            function setupList() {
                var listContainer = listWrapper.querySelector('ul');
                if (!listContainer) return;

                var items = Array.from(listContainer.children).filter(function (el) { return el.tagName === 'LI'; });
                var pagingId = 'comment-paging-' + listIndex;
                var existingPaging = document.getElementById(pagingId);
                ...
                var pagingContainer = existingPaging;
                if (!pagingContainer) {
                    pagingContainer = document.createElement('div');
                    pagingContainer.className = 'paging paging-comment';
                    pagingContainer.id = pagingId;
                    if (listWrapper.parentNode) {
                        listWrapper.parentNode.insertBefore(pagingContainer, listWrapper.nextSibling);
                    }
                }
                pagingContainer.style.display = 'block';
                ...
                function render() {
                    ...
                    // Prev Button
                    var prevClass = (currentPage > 1) ? 'prev' : 'prev disabled';
                    var prevPage = (currentPage > 1) ? (currentPage - 1) : currentPage;
                    html += '<a href="#" data-page="' + prevPage + '" class="' + prevClass + '">&lt;</a>';

                    // Numbers
                    html += '<span class="numbox">';
                    for (var i = 1; i <= totalPages; i++) {
                        if (i === currentPage) {
                            html += '<span class="selected">' + i + '</span>';
                        } else {
                            html += '<a href="#" data-page="' + i + '" class="num">' + i + '</a>';
                        }
                    }
                    html += '</span>';

                    // Next Button
                    var nextClass = (currentPage < totalPages) ? 'next' : 'next disabled';
                    var nextPage = (currentPage < totalPages) ? (currentPage + 1) : currentPage;
                    html += '<a href="#" data-page="' + nextPage + '" class="' + nextClass + '">&gt;</a>';

                    pagingContainer.innerHTML = html;

                    // Bind events
                    var links = pagingContainer.querySelectorAll('a');
            ...
            var targetNode = listWrapper.querySelector('ul') || listWrapper;
    ...
    function submitMobileSearch() {
        var input = document.querySelector('.m-search-form input');
