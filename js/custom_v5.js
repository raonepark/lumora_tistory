  ...
  */
  ...
  (function () {
    ...
    function showInvalid(msg, $textarea, e) {
      ...
      if ($textarea && $textarea.length) $textarea.focus();
      return false;
    }
    ...
    $form.on('submit.validateComment', function (e) {
      ...
      // 정상 제출: 기본 흐름(로그인 확인/리다이렉트)을 그대로 통과
      isSubmitting = true;
      setTimeout(function(){ isSubmitting = false; }, 1500);
    });

    // ❗ 버튼 click 가드는 걸지 않는다 (테마 기본 로직과 중복 방지)
    var $submitBtn = $form.find('input[type="submit"], button[type="submit"]').first();
    if ($submitBtn.length) { $submitBtn.off('.validateComment'); }
  })();
});
